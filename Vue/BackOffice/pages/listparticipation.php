...
<head>
  ...
  <title>EasyParki - Liste Participation</title>
      ...
      <div class="d-flex justify-content-between align-items-center mb-4">
        ...
        <div>
          ...
          <a href="addparticipation.php" class="btn btn-primary">
            <i class="fas fa-plus me-2"></i>Ajouter Participation
          </a>
        </div>
      </div>
        <table class="table table-striped align-middle">
          ...
          <tbody>
            <?php foreach ($liste as $participation): ?>
            <tr>
              <td><?= $participation['id_participation'] ?></td>
              <td><?= $participation['idE'] ?></td>
              <td><?= $participation['nom_participant'] ?></td>
              ...
              <div class="d-flex flex-column gap-2">
                <!-- Bouton Modifier -->
                <form method="POST" action="updateparticipation.php" class="m-0">
                  <input type="hidden" name="id" value="<?= htmlspecialchars($participation['id_participation']) ?>">
                  <button type="submit" class="btn btn-sm btn-info w-100">
                    <i class="fas fa-edit"></i> Modifier
                  </button>
                </form>
                  <!-- Bouton Supprimer -->
                <form method="GET" action="deleteparticipation.php" class="m-0" onsubmit="return confirm('Êtes-vous sûr de vouloir supprimer ce participant ?');">
                  <input type="hidden" name="id" value="<?= htmlspecialchars($participation['id_participation']) ?>">
                  <button type="button" class="btn btn-sm btn-danger w-100" onclick="confirmDelete(event, <?= $participation['id_participation'] ?>)">
